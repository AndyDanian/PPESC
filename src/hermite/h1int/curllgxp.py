...
def curllgxp(
    ...
    """
    Curl Angular moment on x by lineal momentum, which is a vector
    ...
    curllgxp: list = [0 for i in range(int(total_nprim * (total_nprim + 1) / 2))]

    count: int = 0

    """
    Component Selection p x (Lg,x p) = 
                        = dy(ygdz-zgdy)dz-dz(ygdz-zgdy)dy + ...
    ...
    for i in range(total_nprim):
        ...
        for j in range(i, total_nprim):
            ...
            if magnetic_component == 0:
                ...
                d2phi: float = 0.0
                for sij, c, ml in zip([sxx*szz, sxx*syy], [1,2], [ly,lz]):
                    d2phi += sij*(
                                4.0
                                * exp[j]
                                * exp[j]
    ...
    if output > 10:
        driver_time.add_name_delta_time(
            name=f"Curl Angular Momentum Atomic Integrals on X by Lineal Momentum on {magnetic_component} ",
