...
def get_coulomb_exchange_integrals(
    driver_time: drv_time, io: scratch, wf: wave_function, verbose: int = 0
) -> None:
    """
    ...
    mo_coeff: np.ndarray = np.array(wf.mo_coefficients)
    n_mo_occ: int = wf.mo_occ
    n_mo_virt: int = wf.mo_virt

    coulomb: np.ndarray = np.zeros(
        (n_mo_virt, n_mo_virt, n_mo_occ, n_mo_occ), dtype=float
    )
    exchange: np.ndarray = np.zeros(
        (n_mo_virt, n_mo_occ, n_mo_virt, n_mo_occ), dtype=float
    )
    coulomb, exchange = ao2mo(
        moco=np.asfortranarray(mo_coeff),
        a2i=np.asfortranarray(
            io.binary(file=io._hermite_ao2b_binary, io="r", label="e2pot")
        ),
        nocc=n_mo_occ,
        nvir=n_mo_virt,
    )
    ...
    io.binary(file=io._exchange_coulomb, dictionary={"exchange": exchange}, io="a")
    io.binary(file=io._exchange_coulomb, dictionary={"coulomb": coulomb}, io="a")
    ## Write in the ouput the EXCHCOUL.H5 file
    io.write_output(
        information=f"Coulomb and Exchange ({io._exchange_coulomb.name})",
        type=3,
        size_file=io._exchange_coulomb.stat().st_size,
    )

    driver_time.add_name_delta_time(
        name="Exchange and Coulomb", delta_time=(time() - start)
    )
            ...
            for b in range(n_mo_virt):
                for i in range(n_mo_occ):
                    for j in range(n_mo_occ):
                        io.write_output(
                            f"{a + n_mo_occ + 1,b + n_mo_occ + 1,i + 1,j + 1}".center(
                                15
                            )
                            + f"{coulomb[a,b,i,j]:.8f}".center(20)
                            + f"{a + n_mo_occ + 1,i + 1,b + n_mo_occ + 1,j + 1}".center(
                                15
                            )
...
def ao_2_mo(wf: wave_function, verbose: int = 0):
    """
    ...
    mo2i: np.ndarray = np.zeros((nprim, nprim, nprim, nprim), dtype=float)
    count: int = 0
    for a in range(nprim):

        # Transform first index
        f: np.ndarray = np.zeros((nprim, nprim, nprim), dtype=float)
        g: np.ndarray = np.zeros((nprim, nprim, nprim), dtype=float)
