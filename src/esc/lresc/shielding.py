...
def correction_to_calculate(
    lresc_amounts: list[str] = [], tensor: bool = False
    ...
    if not lresc_amounts:
        return (
            ...
            },
            {
                ...
                "triplet_quadratic_amount": True,
                "singlet_quadratic_amount": True,
                "dia_singlet_lineal": True,
            },
            {"dia_nr": True, "dia_avs": True},
        )
    else:
        if "paranr" in lresc_amounts or "nr" in lresc_amounts:
            para_nr = True
        if "dianr" in lresc_amounts or "nr" in lresc_amounts:
            dia_nr = True
        if "diaavs" in lresc_amounts or "avs" in lresc_amounts:
            diaavs = True
        if (
            "fclap" in lresc_amounts
            or "sdlap" in lresc_amounts
            or "fcbso" in lresc_amounts
            or "sdbso" in lresc_amounts
        ):
            triplet_lineal_amount = True
        if "lpsokin" in lresc_amounts or "lkinpso" in lresc_amounts:
            singlet_lineal_amount = True
        if (
            "lfcso" in lresc_amounts
            or "lsdsoxx" in lresc_amounts
            or "lsdsoyy" in lresc_amounts
            or "lsdsozz" in lresc_amounts
        ):
            triplet_quadratic_amount = True
        if "lpsodw" in lresc_amounts or "lpsomv" in lresc_amounts:
            singlet_quadratic_amount = True
        if "a2mv" in lresc_amounts or "a2dw" in lresc_amounts:
            dia_lineal = True
        return (
            {
                "paranr": "paranr" in lresc_amounts,
                # Lineal -- Triplet
                "fclap": "fclap" in lresc_amounts,
                "sddxx": "sddxx" in lresc_amounts,
                "sddyy": "sddyy" in lresc_amounts,
                "sddzz": "sddzz" in lresc_amounts,
                "sddxy": "sddxy" in lresc_amounts,
                "sddxz": "sddxz" in lresc_amounts,
                "sddyx": "sddyx" in lresc_amounts,
                "sddyz": "sddyz" in lresc_amounts,
                "sddzx": "sddzx" in lresc_amounts,
                "sddzy": "sddzy" in lresc_amounts,
                "fcbso": "fcbso" in lresc_amounts,
                "sdbsoxx": "sdbsoxx" in lresc_amounts,
                "sdbsoyy": "sdbsoyy" in lresc_amounts,
                "sdbsozz": "sdbsozz" in lresc_amounts,
                "sdbsoxy": "sdbsoxy" in lresc_amounts,
                "sdbsoxz": "sdbsoxz" in lresc_amounts,
                "sdbsoyx": "sdbsoyx" in lresc_amounts,
                "sdbsoyz": "sdbsoyz" in lresc_amounts,
                "sdbsozy": "sdbsozy" in lresc_amounts,
                "sdbsozx": "sdbsozx" in lresc_amounts,
                # Lineal -- Singlet
                "lpsokin": "lpsokin" in lresc_amounts,
                "lkinpso": "lkinpso" in lresc_amounts,
                # Quadratic -- Triplet
                "lfcso": "lfcso" in lresc_amounts,
                "lsdsoxx": "lsdsoxx" in lresc_amounts,
                "lsdsoyy": "lsdsoyy" in lresc_amounts,
                "lsdsozz": "lsdsozz" in lresc_amounts,
                "lsdsoxy": "lsdsoxy" in lresc_amounts,
                "lsdsoxz": "lsdsoxz" in lresc_amounts,
                "lsdsoyx": "lsdsoyx" in lresc_amounts,
                "lsdsoyz": "lsdsoyz" in lresc_amounts,
                "lsdsozx": "lsdsozx" in lresc_amounts,
                "lsdsozy": "lsdsozy" in lresc_amounts,
                # Quadratic -- Singlet
                "lpsodw": "lpsodw" in lresc_amounts,
                "lpsomv": "lpsomv" in lresc_amounts,
                # Diamagnetic
                # NR
                "dianr": "dianr" in lresc_amounts,
                # Lineal singlet responses
                "a2mv": "a2mv" in lresc_amounts,
                "a2dw": "a2dw" in lresc_amounts,
                # Averages
                "fc": "fc" in lresc_amounts,
                "psooz": "psooz" in lresc_amounts,
                "dnske": "dnske" in lresc_amounts,
            },
            {
                "para_nr": para_nr,
                "triplet_lineal_amount": triplet_lineal_amount,
                "singlet_lineal_amount": singlet_lineal_amount,
                "triplet_quadratic_amount": triplet_quadratic_amount,
                "singlet_quadratic_amount": singlet_quadratic_amount,
                "dia_singlet_lineal": dia_lineal,
            },
            {"dia_nr": dia_nr, "dia_avs": diaavs},
        )


def run_shielding_lresc(
    io: scratch,
    wf: wave_function,
    driver_time: drv_time,
    lresc_amounts: list,
    lresc_consts: dict[str, float],
    atom: list,
    tensor: bool = False,
    ...
    for a in atom:
        ...
        if not tensor:
            ...
            if (
                "sdbsoxx" in atom_responses.keys()
                and "sdbsoyy" in atom_responses.keys()
                and "sdbsozz" in atom_responses.keys()
            ):
            ...
            if (
                "lsdsoxx" in atom_responses.keys()
                and "lsdsoyy" in atom_responses.keys()
                and "lsdsozz" in atom_responses.keys()
            ):
            ...
            if (
                "sdbsoxx" in atom_responses.keys()
                and "sdbsoxy" in atom_responses.keys()
                and "sdbsoxz" in atom_responses.keys()
                and "sdbsoyx" in atom_responses.keys()
                and "sdbsoyy" in atom_responses.keys()
                and "sdbsoyz" in atom_responses.keys()
                and "sdbsozx" in atom_responses.keys()
                and "sdbsozy" in atom_responses.keys()
                and "sdbsozz" in atom_responses.keys()
            ):
                atom_responses["sdbso"] = [
                    sum(atom_responses["sdbsoxx"]),
            ...
            if (
                "lsdsoxx" in atom_responses.keys()
                and "lsdsoxy" in atom_responses.keys()
                and "lsdsoxz" in atom_responses.keys()
                and "lsdsoyx" in atom_responses.keys()
                and "lsdsoyy" in atom_responses.keys()
                and "lsdsoyz" in atom_responses.keys()
                and "lsdsozx" in atom_responses.keys()
                and "lsdsozy" in atom_responses.keys()
                and "lsdsozz" in atom_responses.keys()
            ):
                atom_responses["lsdso"] = [
                    sum(atom_responses["lsdsoxx"]),
    ...
    if verbose > 10:
        ...
        driver_time.add_name_delta_time(
            name="LRESC Amounts Calculations", delta_time=(time() - start)
        )
...
def get_shielding_iso_ani(
    ...
    for atom, amounts in all_responses.items():
        isotropic: dict[str, float] = {}
        anisotropic: dict[str, float] = {}
    ...
    for atom in isotropic_responses.keys():
        ...
        if verbose > 10:
            ...
            if "paranr" in isotropic_responses[atom].keys():
                ...
                io.write_output(
                    information=[lresc_label["paranr"]],
                    type=4,
                    values=[
...
def print_lresc_tensor(
    ...
    for atom in responses_tensor.keys():
        ...
        values: list = [
            value
            for name, value in responses_tensor[atom].items()
            if name != "paranr" and name != "a2mv" and name != "a2dw"
        ]
        if len(names) > 0:
